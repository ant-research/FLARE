...
from models.blocks import Block, DecoderBlock, PatchEmbed, DecoderBlock_onlyself
...
class CroCoNet(nn.Module):
    ...
    def __init__(self,
        ...
        self.enc_fine_norm = norm_layer(enc_embed_dim // 64)
        self.enc_fine_norm2 = norm_layer(enc_embed_dim)
        self.enc_blocks_stage2 = nn.ModuleList([
            Block(enc_embed_dim, enc_num_heads, mlp_ratio, qkv_bias=True, norm_layer=norm_layer, rope=self.rope)
            for i in range(enc_depth//6)])
        self.enc_norm_stage2 = norm_layer(enc_embed_dim)  
        self.decoder_embed_app = nn.Linear(enc_embed_dim, dec_embed_dim, bias=True)
        # masked tokens 
    ...
    def _set_decoder(self, enc_embed_dim, dec_embed_dim, dec_num_heads, dec_depth, mlp_ratio, norm_layer, norm_im2_in_dec):
        ...
        self.dec_blocks3 = nn.ModuleList([
            DecoderBlock(dec_embed_dim, dec_num_heads, mlp_ratio=mlp_ratio, qkv_bias=True, norm_layer=norm_layer, norm_mem=norm_im2_in_dec, rope=self.rope)
            for i in range(dec_depth)])
        self.dec_blocks4 = nn.ModuleList([
            DecoderBlock(dec_embed_dim, dec_num_heads, mlp_ratio=mlp_ratio, qkv_bias=True, norm_layer=norm_layer, norm_mem=norm_im2_in_dec, rope=self.rope)
            for i in range(dec_depth)])

        self.dec_blocks_add3 = nn.ModuleList([
            DecoderBlock(dec_embed_dim, dec_num_heads, mlp_ratio=mlp_ratio, qkv_bias=True, norm_layer=norm_layer, norm_mem=norm_im2_in_dec, rope=self.rope)
            for i in range(3)])
        self.dec_blocks_add4 = nn.ModuleList([
            DecoderBlock(dec_embed_dim, dec_num_heads, mlp_ratio=mlp_ratio, qkv_bias=True, norm_layer=norm_layer, norm_mem=norm_im2_in_dec, rope=self.rope)
            for i in range(3)])
        self.fusion_add1 = nn.ModuleList([nn.Linear(dec_embed_dim, dec_embed_dim, bias=False) for i in range(3)])
        self.fusion_add2 = nn.ModuleList([nn.Linear(dec_embed_dim, dec_embed_dim, bias=False) for i in range(3)])
        # final norm layer 
        ...
        self.cam_cond_embed1_stage2 = nn.ModuleList([nn.Linear(dec_embed_dim, dec_embed_dim, bias=False) for i in range(dec_depth)])
        self.cam_cond_embed2_stage2 = nn.ModuleList([nn.Linear(dec_embed_dim, dec_embed_dim, bias=False) for i in range(dec_depth)])
        self.pre_cond_embed1_stage2 = nn.ModuleList([nn.Linear(dec_embed_dim, dec_embed_dim, bias=False) for i in range(dec_depth)])
        self.pre_cond_embed2_stage2 = nn.ModuleList([nn.Linear(dec_embed_dim, dec_embed_dim, bias=False) for i in range(dec_depth)])
        self.fusion1 = nn.ModuleList([nn.Linear(dec_embed_dim, dec_embed_dim, bias=False) for i in range(dec_depth)])
        self.fusion2 = nn.ModuleList([nn.Linear(dec_embed_dim, dec_embed_dim, bias=False) for i in range(dec_depth)])
        ...
        self.dec_stage2_norm = norm_layer(dec_embed_dim)
    ...
    def _init_weights(self, m):
        ...
        if m == self.decoder_embed_app:
            m.weight.data.fill_(0.)
            m.bias.data.fill_(0.)
        elif m in self.cam_cond_embed1_stage2:
            m.weight.data.fill_(0.)
        elif m in self.cam_cond_embed2_stage2:
            m.weight.data.fill_(0.)
        elif m in self.pre_cond_embed1_stage2:
            m.weight.data.fill_(0.)
        elif m in self.pre_cond_embed2_stage2:
            m.weight.data.fill_(0.)
        elif m in self.fusion2:
            m.weight.data.fill_(0.)
        elif m in self.fusion1:
            m.weight.data.fill_(0.)
        elif m in self.fusion_add1:
            m.weight.data.fill_(0.)
        elif m in self.fusion_add2:
            m.weight.data.fill_(0.)
