...
class DL3DV_test(BaseStereoViewDataset_test):
    def __init__(self, *args, split, ROOT, only_pose=False, scene=None, **kwargs):
        self.ROOT = ROOT
        self.json_paths = os.listdir(ROOT)
        self.scene = scene
        # self.json_paths = ['e6ed4dc618717f43d409a9243b5501a8ff2f5725a64d9091c843b7d2f6fedc56', ]
    ...
    def sequential_sample(self, im_start, last, interal):
        ...
        im_list += [
            random.choice(im_list) + random.choice(list(range(-interal, interal)))
    ...
    def _get_views(self, idx, resolution, rng):
        ...
        if self.scene is None:
            scene = rng.choice(self.json_paths)
        else:
            scene = self.scene
        json_path = scene
        ...
        while len(imgs_idxs) > 0:  # some images (few) have zero depth
            ...
            if not os.path.exists(depth_path):
                depth_path = osp.join(self.ROOT, scene_name, 'depth', '{:05d}.npy'.format(frame_num))
            depthmap = np.load(depth_path)
            mask_path = osp.join(self.ROOT, scene_name, 'filter', 'mask', '{:05d}_final.png'.format(frame_num))
            if not os.path.exists(mask_path):
                mask_path = osp.join(self.ROOT, scene_name, 'mask', '{:05d}_final.png'.format(frame_num))
            ...
            orig_shape = depth_anything.shape
            ...
            _, _, intrinsics_256, _ = self._crop_resize_if_necessary(
                rgb_image, depthmap, intrinsics.copy(), (256,256), rng=rng, info=None, depth_anything=depth_anything)
            fxfycxcy_256 = np.array([intrinsics_256[0, 0]/256, intrinsics_256[1, 1]/256, intrinsics_256[0,2]/256, intrinsics_256[1,2]/256]).astype(np.float32)
            views.append(dict(
