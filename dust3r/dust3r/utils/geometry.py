...
def closed_form_inverse_OpenCV(se3, R=None, T=None):
    ...
    | 0       1  |
    """
    # Extract the rotation matrix R and translation vector T if not provided
    if R is None:
        R = se3[:, :3, :3]
    ...
    # Compute the transpose of the rotation matrix R
    R_transposed = R.transpose(1, 2)

    # Calculate the top-right part of the inverse matrix (-R^T * t)
    top_right = -R_transposed.bmm(T)

    # Initialize the inverted matrix as an identity matrix for each SE3 matrix
    inverted_matrix = torch.eye(4, 4)[None].repeat(se3.size(0), 1, 1).to(R.dtype).to(R.device)

    # Populate the rotation and translation components in the inverted matrix
...
def normalize_pointcloud(pts1, pts2, norm_mode='avg_dis', valid1=None, valid2=None, ret_factor=False):
    ...
    if norm_mode == 'avg':
        ...
        norm_factor = all_dis.sum(dim=1) / (nnz1 + nnz2 + 1e-7)
    else:
        # gather all points together (joint normalization)
