...
def loss_of_one_batch_2v(gt_num_image, batch, model, criterion, device, symmetrize_batch=False, use_amp=False, ret=None):
    if bool(use_amp):
    ...
    with torch.cuda.amp.autocast(enabled=bool(use_amp), dtype=dtype):
        views = batch
        ignore_keys = set(['depthmap', 'dataset', 'label', 'instance', 'idx', 'true_shape', 'rng', 'overlap'])
...
def loss_of_one_batch(gt_num_image, batch, model, criterion, device, symmetrize_batch=False, use_amp=False, ret=None):
    ...
    with torch.cuda.amp.autocast(enabled=bool(use_amp), dtype=dtype):
        views = batch
        ignore_keys = set(['depthmap', 'dataset', 'label', 'instance', 'idx', 'true_shape', 'rng', 'vid'])
        ignore_dtype_keys = set(['camera_pose', 'pts3d', 'fxfycxcy', 'img_org', 'camera_intrinsics', 'depthmap', 'depth_anything', 'fxfycxcy_unorm'])
        ...
        elif gt_num_image == 32:
            view2 = views[1:-gt_num_image]
            render_gt = views[-gt_num_image:]
        else:
            view2 = views[1:-gt_num_image]
